...
async function getdata(lat,lon){  
    ...
    try{
     ...
     // Hide the search elements after successful city selection
     hideSearchElements();
}
...
async function search_val() {
    ...
    let url = `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&APPID=5917703f82eb1704672b42592e3116c7`;
    try {
        let res = await fetch(url);
        let data = await res.json();
...
// Function to hide search elements
...
if(data.current.weather[0].main=="Haze"){
    ...
    back.style.backgroundImage = "url('https://i.gifer.com/AG70.gif')"
}
